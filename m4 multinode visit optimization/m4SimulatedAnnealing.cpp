...
//    }
    ...
    long double temperature = 1000000000;
    long double coolingConstant = 0.999995;
 ...
 float truck_capacity, const vector<DeliveryInfo>& deliveries, const std::vector<unsigned>& depots) {
   ...
   //we've found the distance of the original deliveryOrder vector... it's bad, so call it worstDistance
    ...
    seed = std::chrono::system_clock::now().time_since_epoch().count();
                ...
                std::uniform_real_distribution<long double> distribution(0.1, 100000000);
...
//                deliveryOrder = swap;
                ...
                randomNumber = (distribution(generator))*10;
                ...
                if (legal) {
                    ...
                    acceptanceProbability = (exp(-delta/currentTemp))*10*10*10*10*10*10*10;
    ...
    outputPath.clear();
    std::cout << "testing if this is empty... " << std::endl;
    if (bestDeliveryOrder.empty()) std::cout << "yep it is empty" << std::endl;
    else std::cout << "nope it isn't empty" << std::endl;
